...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Run Squirrel Finger Parallel with Ray.")
    parser.add_argument(
        "--test_mode",
        type=str,
        default="full_sweep",
        choices=["full_sweep", "curvature"],
                        help="Choose which parameter sweep to run.")
    parser.add_argument(
        "--num_cpus",
        type=int,
        default=None,
        help="Override how many CPU cores Ray should use.",
    )
    ...
    if failures > 0:
        ...
        for success, p, err in results:
            if not success:
                print(f"[-] Crash @ Deg:{p.get('approach_deg')} T:{p.get('tension')} VM:{p.get('v_mode')}")
