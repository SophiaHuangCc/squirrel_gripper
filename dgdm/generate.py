...
def main():
    ...
    if not dyn_ckpt.get("noise_conditioned", False):
        raise ValueError("DGDM dynamics checkpoint must be trained with diffusion noise and timestep conditioning")
    ...
    dynamics_steps=int(dyn_ckpt.get("num_train_timesteps",dyn_ckpt.get("args",{}).get("timesteps",100)))
    if dynamics_steps != train_steps:
        raise ValueError(f"Prior uses {train_steps} timesteps but dynamics uses {dynamics_steps}")
    ...
    with torch.no_grad(): scores=aggregate_profile_score(dynamics,out["design_unit"],scenarios,target,torch.tensor(0.0,device=device))
